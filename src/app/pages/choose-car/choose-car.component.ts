...
import { ColorDTO, ModelDTO } from '../../types/types'
...
export class ChooseCarComponent implements OnInit {
  ...
  subscribeToFormStatusChanges() {
    this.chooseCarForm.statusChanges.subscribe(status => {
      if(this.chooseCarForm.dirty) {
        switch(status) {
          case 'VALID':
            const chosenModel = this.models.find(({code}) => code === this.chooseCarForm.get('modelSelect')?.value)
  ...
  initColorsArray(modelCode: string | null) {
    const newColors = this.models.find(({code}) => code === modelCode)?.colors
    if (newColors) {
