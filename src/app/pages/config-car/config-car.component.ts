...
import { ConfigurationDTO, TypoDTO } from '../../types/types'
...
export class ConfigCarComponent implements OnInit {
  ...
  actualConfiguration?: TypoDTO

  chooseConfigurationForm = new FormGroup({
    configSelect: new FormControl(-1, [Validators.required, Validators.min(0)])
  });

  constructor(private router: Router, private apiGate: ApiGateService, protected state: StateService) {}
  ...
  subscribeToConfigs() {
    this.apiGate.getConfigurations(this.state.getSummary().code!).subscribe(data => {
  ...
  initByState() {
    ...
    this.chooseConfigurationForm.setValue({configSelect: summary.config?.id!})
  }

  subscribeToConfigSelectChanges() {
    this.chooseConfigurationForm.controls['configSelect'].valueChanges.subscribe(value => {
      this.actualConfiguration = this.configuration?.configs.find(({id}) => id == value)
    });
  }
